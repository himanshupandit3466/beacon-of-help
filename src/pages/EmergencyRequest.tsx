...
import { createHelpRequest } from '@/services/emergencyService';
  ...
  const handleSubmit = async () => {
    ...
    if (!user) {
      toast({
        ...
        description: "Please sign in to send a help request",
        variant: "destructive"
      });
      navigate('/login');
      return;
    ...
    try {
      // Create help request in Supabase
      const requestDetails = await createHelpRequest(
        type || 'other',
        location,
      ...
      navigate(`/request-tracking/${requestDetails.id}`);
    } catch (error: any) {
